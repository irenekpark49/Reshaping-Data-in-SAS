...
title "NACC Dataset";
proc print data=homework1 (obs=50)
...
proc transpose data=homework1 out=mmse prefix=mmse;
	by id;
	id visit;
	var mmse_score;
run;
...
proc transpose data=homework1 out=immediate prefix=immediate;
	by id;
	id visit;
	var immediate_score;
run;
...
proc transpose data=homework1 out=delayed prefix=delayed;
	by id;
	id visit;
	var delayed_score;
run;
...
proc transpose data=homework1 out=digit prefix=digit;
	by id;
	id visit;
	var digit_forward;
run;
...
proc transpose data=homework1 out=&dataset_name prefix=&psych_prefix;
	by id;
	id visit;
	var &psych_measure; 
run;
